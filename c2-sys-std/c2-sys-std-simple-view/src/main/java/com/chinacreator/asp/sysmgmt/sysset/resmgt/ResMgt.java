...
import com.chinacreator.asp.sysmgmt.sysset.jobresmgt.JobResMgtMessages;
			...
			if ("add".equals(formType)) {
				if ("privilegeName_Field".equals(elementId)) {
					if (menuService.existsByMenuName(elementValue,
							menuDTO.getParentId())) {
						validate = "error";
						errmess = JobResMgtMessages
								.getString("JOBRESMGT.PRIVILEGENAME_IS_EXISTS");
					}
				} else if ("privilegeCode_Field".equals(elementId)) {
					if (menuService.existsByMenuCode(elementValue)) {
						validate = "error";
						errmess = JobResMgtMessages
								.getString("JOBRESMGT.PRIVILEGECODE_IS_EXISTS");
					}
				}
