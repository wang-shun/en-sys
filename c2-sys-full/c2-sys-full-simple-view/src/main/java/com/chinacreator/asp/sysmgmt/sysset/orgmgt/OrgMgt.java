...
public class OrgMgt {
	...
	public Map<String, String> validateFormByOrg(String elementId, String elementValue, String formType,
			OrgDTO orgDTO) {
		Map<String, String> map = new HashMap<String, String>();
		String validate = "success";
		String errmess = "";
		if (null != elementId && !elementId.trim().equals("") && null != elementValue && !elementValue.trim().equals("")
				&& null != formType && !formType.trim().equals("") && null != orgDTO) {
			...
			if ("add".equals(formType)) {
				if ("orgNumber".equals(elementId)) {
					if (orgService.existsByOrgNumber(elementValue)) {
						validate = "error";
						errmess = OrgMessages.getString("ORG.ORG_NUMBER_EXISTED");
					}
				} else if ("orgName".equals(elementId)) {
					if (CommonPropertiesUtil.isUniqueOrgName() && orgService.existsByOrgName(elementValue)) {
						validate = "error";
					...
					if (CommonPropertiesUtil.isUniqueOrgShowName()
							&& orgService.existsByOrgShowName(orgDTO.getParentId(), elementValue)) {
						validate = "error";
						errmess = OrgMessages.getString("ORG.ORG_SHOWNAME_EXISTED_IN_THE_SAME_LEVEL");
					}
				}
			} else if ("edit".equals(formType)) {
				if ("orgNumber".equals(elementId)) {
					if (orgService.existsByOrgNumberIgnoreOrgID(elementValue, orgDTO.getOrgId())) {
						validate = "error";
						errmess = OrgMessages.getString("ORG.ORG_NUMBER_EXISTED");
					}
				} else if ("orgName".equals(elementId)) {
					if (CommonPropertiesUtil.isUniqueOrgName()
							&& orgService.existsByOrgNameIgnoreOrgID(elementValue, orgDTO.getOrgId())) {
						validate = "error";
						errmess = OrgMessages.getString("ORG.ORG_NAME_EXISTED");
					}
				} else if ("orgShowName".equals(elementId)) {
					if (CommonPropertiesUtil.isUniqueOrgShowName()
							&& orgService.existsByOrgShowNameIgnoreOrgID(elementValue, orgDTO.getOrgId())) {
						validate = "error";
						errmess = OrgMessages.getString("ORG.ORG_SHOWNAME_EXISTED_IN_THE_SAME_LEVEL");
					}
				}
	...
	public void moveOrgs(String[] orgIds, String parentId, String... sortOrgIds) {
		if (null != orgIds && orgIds.length > 0 && null != parentId && !parentId.trim().equals("")) {
	...
	@Transactional(CommonConstants.sfs_SYSMGT_TRANSACTIONMANAGER_NAME)
	public void orgSetRoles(String[] orgIds, String[] addRoleIds, String[] delRoleIds) {
		orgIds = toRepeat(orgIds);
		addRoleIds = toRepeat(addRoleIds);
		delRoleIds = toRepeat(delRoleIds);
		
		if(orgIds.length>0){
			if(delRoleIds.length>0){
				roleService.revokeFromOrgs(delRoleIds, orgIds);
			}
			if(addRoleIds.length>0){
				roleService.assignToOrgs(addRoleIds, orgIds);
			}
		}
	}
	...
	private String generateOrgNumber() {
		...
		try {
			GenerateOrgNumberSpi generateOrgNumberSpi = ApplicationContextManager.getContext()
					.getBean(GenerateOrgNumberSpi.class);
	...
	private String generateOrgXzqm() {
		String orgXzqm = null;
		try {
			GenerateOrgXzqmSpi generateOrgXzqmSpi = ApplicationContextManager.getContext()
					.getBean(GenerateOrgXzqmSpi.class);
