...
public class RoleResTreeContentProviderImpl implements TreeContentProvider {
	...
	private AccessControlService accessControlService = ApplicationContextManager.getContext()
	...
	public TreeNode[] getElements(TreeContext context) {
		...
		if (null != context) {
			...
			if (null == resId || resId.trim().equals("") || resId.trim().equals("other") || null == resourceTreeNode
					|| null == menuList || "true".equals(isLoad)) {
				resMap = new HashMap<String, Object>();
				ResourceTreeNodeBuilder builder = new ResourceTreeNodeBuilder(allPrivilegeDTOList());
			...
			if ("0".equals(resId)) {
				ResourceTreeNode menuTreeNode = new ResourceTreeNode("0", "menu", "菜单", "menu", true);
					...
					for (PrivilegeDTO privilegeDTO : menuList) {
						if (selResId.equals(privilegeDTO.getParentId())) {
							ResourceTreeNode menuTreeNode = new ResourceTreeNode(
									"0".equals(selResId) ? "menu" : privilegeDTO.getParentId(),
									privilegeDTO.getPrivilegeId(), privilegeDTO.getPrivilegeName(), "menu", false);
							menuTreeNode.setParent(isParentByMenu(privilegeDTO.getPrivilegeId(), menuList));
							menuTreeNode.setPrivilegeId(privilegeDTO.getPrivilegeId());
							menuTreeNode.setNoteTitle(getTitle(privilegeDTO.getPrivilegeName(), "menu"));
							menuTreeNode.setIcon("ext/resTreeIcons/menu.png");
							menuTreeNode.setObj(resMap.get(privilegeDTO.getPrivilegeId()));
							...
							if (null != presetId && !presetId.trim().equals("")) {
								...
								if (presetIdSet.contains(roleService.getAdministratorRoleId())) {
									...
									for (String id : presetIdSet) {										
										if (roleService.hasPrivilege(id, privilegeDTO.getPrivilegeId())) {
											checked = true;
											break;
										}
									}
							...
							if (!checked && null != roleIds && roleIds.length == 1) {
								if (roleService.hasPrivilege(roleIds[0], privilegeDTO.getPrivilegeId())) {
									checked = true;
								}
							}
			...
			if ("0".equals(resId) || !"menu".equals(type)) {
				if (null != resourceTreeNode && resourceTreeNode.length > 0) {
					for (TreeNode treeNode : resourceTreeNode) {
						ResourceTreeNode node = new ResourceTreeNode(treeNode.getId(), treeNode.getName());
						BeanCopierUtil.copy(((ResourceTreeNode) treeNode), node);
						if (resId.equals(node.getPid())) {
							if (node.isVirtual()) {
								...
								node.setNoteTitle(getTitle(treeNode.getName(), node.getType()));
								node.setName(getNameByNodeName(treeNode.getName()));
								node.setIcon("ext/resTreeIcons/" + node.getType() + ".png");
								...
								if (null != presetId && !presetId.trim().equals("")) {
									...
									if (presetIdSet.contains(roleService.getAdministratorRoleId())) {
										...
										for (String id : presetIdSet) {
											if (roleService.hasPrivilege(id, node.getPrivilegeId())) {
												checked = true;
												break;
											}
										}
								...
								if (!checked && null != roleIds && roleIds.length == 1) {
									if (roleService.hasPrivilege(roleIds[0], node.getPrivilegeId())) {
										checked = true;
									}
								}
	...
	private List<PrivilegeDTO> allPrivilegeDTOList() {
		...
		if (null != privilegeDTOList && !privilegeDTOList.isEmpty()) {
			for (PrivilegeDTO privilegeDTO : privilegeDTOList) {
				if (accessControlService.isPermitted(privilegeDTO.getPermExpr())) {
			...
			for (PrivilegeDTO privilegeDTO : privilegeDTOList) {
				if (accessControlService.isPermitted(privilegeDTO.getPermExpr())) {
					if ("4".equals(privilegeDTO.getType())) {
						...
						resMap.put(privilegeDTO.getPrivilegeId(), menuService.queryByPK(privilegeDTO.getPrivilegeId()));
					}
				}
			}
		}
	...
	private boolean isParentByMenu(String menuId, List<PrivilegeDTO> menuList) {
		if (null != menuList && !menuList.isEmpty() && null != menuId && !menuId.trim().equals("")) {
			for (PrivilegeDTO privilegeDTO : menuList) {
