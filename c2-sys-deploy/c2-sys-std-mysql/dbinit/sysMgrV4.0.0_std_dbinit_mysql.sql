...
drop procedure log_backup;

drop function org_getChilds;

drop function org_getParents;

drop function privilege_getChilds;

drop function privilege_getParents;
...
(
   ...
   creator              varchar(50) default '1' comment '菜单创建者',
...
(
   ...
   parent_id            varchar(50) not null default '0' comment '父ID',
   type                 char(1) not null default '0' comment '类型（0：功能组，1：静态操作，2：查询策略，3：决策策略，4：菜单）',
...
(
   ...
   info_name            varchar(50) comment '系统信息名称',
   info_content         varchar(200) comment '系统信息内容',
   info_desc            varchar(100) comment '系统信息描述',
...
(
   ...
   type_name            varchar(50) comment '系统信息类型名称',
...
(
   ...
   dictdata_name        varchar(100) comment '字典名称',
   dictdata_value       varchar(100) comment '字典的值',
...
(
   ...
   log_operuser         varchar(50) comment '用户账号',
   op_orgid             varchar(50) comment '操作员所属机构ID',
   oper_module          varchar(200) comment '日志模块名称',
   ...
   oper_type            decimal(1) comment '操作类型
            1:新增
            2:删除
            3:修改
            4:其他',
   remark1              varchar(100) comment '备注',
   primary key (log_id)
);
...
(
   ...
   log_operuser         varchar(50) comment '用户账号',
   op_orgid             varchar(50) comment '操作员所属机构ID',
   oper_module          varchar(200) comment '系统中的操作所属的菜单id保持一致',
   log_visitorial       varchar(200) comment '操作来源（一般为操作员机器ip）',
   log_opertime         timestamp default current_timestamp comment '操作时间',
   log_content          text comment '操作内容
            操作内容格式：
            （1）批量操作型格式：将操作的输入值作为操作内容。格式：输入项1：输入值1；输入项2：输入值2……
            （2）单数据操作型格式：登记户ID（或身份ID）：XXXX；登记户名称：XXXXX；数据的关键值（风格与详细信息风格一致）。
            注：申报征收中，凭证种类、应补税额、税种税目是关键值。',
   oper_type            decimal(1) comment '操作类型
            1:新增
            2:删除
            3:修改
...
(
   ...
   org_xzqm             varchar(10) comment '行政区码',
...
-- 权限及菜单
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('1', 'myPanel', '我的面板', '0', '0', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('2', 'sysSet', '系统设置', '0', '0', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('3', 'logMgt', '日志管理', '0', '0', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('4', 'aspMgt', '平台管理', '0', '0', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('11', 'personInfo', '个人信息', '1', '4', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('12', 'modifyPwd', '密码修改', '1', '4', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('21', 'jobMenuMgt', '岗位菜单', '2', '4', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('22', 'orgUserMgt', '机构用户', '2', '4', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('23', 'dictMgt', '字典管理', '2', '4', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('31', 'logQuery', '日志查询', '3', '4', null);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr) values ('41', 'sysInfo', '全局参数配置', '4', '4', null);

insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('1', '1', null, 'fa-dashboard', '1', current_timestamp, 1);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('2', '1', null, 'fa-wrench', '1', current_timestamp, 2);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('3', '1', null, 'fa-book', '1', current_timestamp, 3);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('4', '1', null, 'fa-cogs', '1', current_timestamp, 4);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('11', '1', '#/f/personInfo', 'fa-user', '1', current_timestamp, 1);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('12', '1', '#/f/modifyPwd', 'fa-key', '1', current_timestamp, 2);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('21', '1', '#/f/jobMenuMain', 'fa-sitemap', '1', current_timestamp, 1);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('22', '1', '#/f/orgUserMain', 'fa-users', '1', current_timestamp, 2);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('23', '1', '#/f/dictMain', 'fa-list', '1', current_timestamp, 3);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('31', '1', '#/f/logQueryMain', 'fa-bookmark', '1', current_timestamp, 1);
insert into tb_sm_menu (menu_id, is_enabled, href, icon, creator, creator_time, sn) values ('41', '1', '#/f/sysInfoMain', 'fa-flask', '1', current_timestamp, 1);
