-- 系统管理V4.0.2-4.1.0标准版增量脚本_oracle

-- 字典数据表中字段修改说明
comment on column td_sm_dictdata.dictdata_name is '字典真实值';
comment on column td_sm_dictdata.dictdata_value is '字典显示值';
...
-- 菜单中新增字段（显示方式）
alter table tb_sm_menu add display_mode char(1) default '0' not null; 
comment on column tb_sm_menu.display_mode is '显示方式（0：div中，1：iframe中）';

-- 权限表中字段（权限编码、权限名称、父ID）长度修改为500
alter table tb_sm_privilege modify code varchar2(500);
alter table tb_sm_privilege modify name varchar2(500);
alter table tb_sm_privilege modify parent_id varchar2(500);

-- 权限表中字段（类型）修改字段默认值及说明
alter table tb_sm_privilege modify type default null;
comment on column tb_sm_privilege.type is '类型（1:服务(service),2:表单(form),3:表单元素(dom),4:菜单(menu),5:实体(entity),6:实体操作(entity_op),9:自定义(custom)）';

-- 权限表中新增字段（创建者、创建时间、排序号、权限来源）
alter table tb_sm_privilege add creator varchar2(50) default '1';
alter table tb_sm_privilege add creator_time date default sysdate;
alter table tb_sm_privilege add sn number;
alter table tb_sm_privilege add source char(1) default '0';
comment on column tb_sm_privilege.creator is '创建者';
comment on column tb_sm_privilege.creator_time is '创建时间';
comment on column tb_sm_privilege.sn is '排序号';
comment on column tb_sm_privilege.source is '权限来源（0：自定义，1：IDE）';
...
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr, creator, creator_time, sn) values ('121', 'jobPermMgt', '岗位资源管理', '12', '4', 'jobPermMgt', '1', sysdate, 1);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr, creator, creator_time, sn) values ('122', 'orgUserMgt', '机构用户管理', '12', '4', 'orgUserMgt', '1', sysdate, 2);
insert into tb_sm_privilege (id, code, name, parent_id, type, perm_expr, creator, creator_time, sn) values ('123', 'dictMgt', '字典管理', '12', '4', 'dictMgt', '1', sysdate, 3);
