...
public class UserServiceImpl extends
		...
		for (String jobId : jobIds) {
			...
			for (String userId : userIds) {
				...
				if (!belongsJob(userId, jobId)) {
					...
					// 插入用户与缺省角色关系
					assignRole(userId, roleService.getRoleofeveryoneRoleId(),
							2, jobId);
	...
	public void setJobs(String[] userIds, String[] jobIds) {
		...
		for (String userId : userIds) {
			...
			if (!userJobIdList.isEmpty()) {
				removeJobs(new String[] { userId },
						userJobIdList.toArray(new String[userJobIdList.size()]));
			}

			// 添加用户与岗位的关系
			if (!addjobIdList.isEmpty()) {
				addJobs(new String[] { userId },
	...
	public void removeAllJobs(String... userIds) {
		if (null != userIds && userIds.length > 0) {
			...
			List<UserInstanceEO> userInstanceEOList = userInstanceDao
					.queryByScopeTypeUserIds("2", userIds);
			...
			String[] userinstanceIds = userInsIdList
					.toArray(new String[userInsIdList.size()]);
	...
	public void removeJobs(String[] userIds, String[] jobIds) {
		...
		for (String jobId : jobIds) {
			String[] uiIds = userInstanceUtil
					.getUserInstanceIdByUserIdAndScope(userIds, 2, jobId);
	...
	public void removeFromAllOrgs(String... userIds) {
		...
		for (String userId : userIds) {
			...
			if (null != userInstanceEOList && !userInstanceEOList.isEmpty()) {
				for (UserInstanceEO userInstanceEO : userInstanceEOList) {
					removeAllJobs(userInstanceEO.getScopeId(),
							new String[] { userId });
				}
			}
		}
	...
	public List<UserDTO> queryByOrgJob(UserDTO userDto, String orgId,
		...
		if (null == orgId && null == jobId) {
			...
			List<UserEO> userEOList = userAdvancedDao.queryByOrgJob(userEO,
					orgId, jobId);
	...
	public Page<UserDTO> queryByOrgJob(UserDTO userDto, String orgId,
		...
		if (null == orgId && null == jobId) {
			...
			UserEO userEO = new UserEO();
			BeanCopierUtil.copy(userDto, userEO);
			Page<UserEO> userEOPage = userAdvancedDao.queryByOrgJob(userEO,
					orgId, jobId, pageable, sortable);
	...
	public Page<UserDTO> queryByJob(UserDTO userDto, String jobId,
		...
		UserEO userEO = new UserEO();
		BeanCopierUtil.copy(userDto, userEO);
		Page<UserEO> userEOPage = userAdvancedDao.queryByJob(userEO, jobId,
				pageable, sortable);
