...
public class AddRoleType_TypeNameIsAgain extends TestCase{
	Logger logger = LoggerFactory.getLogger(AddRoleType_TypeNameIsAgain.class);
	@Test
	public void testAddRoleType_TypeNameIsAgain() throws Exception{
		//选择角色类型树根节点
		...
		TestAll.driver.findElement(By.id("typeName")).sendKeys("一般类型");
...
//		    }
		...
		if (webElement.getAttribute("class").contains("message-error"))
			logger.info("异常流-新增角色类型-类型名称重复： 操作成功@@");
		else if (webElement.getAttribute("class").contains("message-success"))
			{
			logger.error("异常流-新增角色类型-类型名称重复： 操作失败!!!");
			Common.TakePic();
			}
