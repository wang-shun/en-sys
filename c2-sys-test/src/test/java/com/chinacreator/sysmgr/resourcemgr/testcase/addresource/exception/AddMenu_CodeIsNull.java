...
public class AddMenu_CodeIsNull extends TestCase{
	Logger logger = LoggerFactory.getLogger(AddMenu_CodeIsNull.class);
	@Test
	public void testAddMenu_CodeIsNull() throws Exception{
		//选择父级菜单
		...
		TestAll.driver.findElement(By.id("privilegeCode_Field")).sendKeys("");
		
		//资源名称
		TestAll.driver.findElement(By.id("privilegeName_Field")).clear();
		TestAll.driver.findElement(By.id("privilegeName_Field")).sendKeys("div中打开se");
...
//		    }
		...
		//判断
		WebElement webElement = TestAll.driver.findElement(By.id("privilegeCode_FieldWrapper"));
		if (webElement.getAttribute("class").contains("has-error"))
			{
			logger.info("异常流-新增菜单-资源编码为空: 操作成功@@");
			}
		else 
			{
			logger.error("异常流-新增菜单-资源编码为空： 操作失败!!!");
			Common.TakePic();
			}
