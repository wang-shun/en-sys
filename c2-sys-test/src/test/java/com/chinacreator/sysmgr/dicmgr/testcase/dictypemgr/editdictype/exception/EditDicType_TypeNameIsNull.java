...
public class EditDicType_TypeNameIsNull extends TestCase{
	Logger logger = LoggerFactory.getLogger(EditDicType_TypeNameIsNull.class);
	@Test
	public void testEditDicType_TypeNameIsNull() throws Exception {
		//选择要编辑的类型
...
//		    }
		...
		if (webElement.getAttribute("class").contains("message-error"))
			logger.info("异常流-编辑字典类型-类型名称为空： 操作成功@@");
		else if (webElement.getAttribute("class").contains("message-success"))
			{
			logger.error("异常流-编辑字典类型-类型名称为空： 操作失败!!!");
			Common.TakePic();
			}
