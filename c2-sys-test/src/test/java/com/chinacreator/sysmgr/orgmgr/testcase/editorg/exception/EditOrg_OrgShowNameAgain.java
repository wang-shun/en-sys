...
public class EditOrg_OrgShowNameAgain extends TestCase{
	Logger logger = LoggerFactory.getLogger(EditOrg_OrgShowNameAgain.class);
	@Test
	public void testEditOrg_OrgShowNameAgain() throws Exception{
		// 修改后信息
	    String orgNumber = "test";
	    String orgName = "test";
	    String orgShowName = "长沙市se";
	    String orgXzqm = "0731";
...
//  		    }
 	   ...
 	   //判断alert为正确弹框还是错误弹框
  		...
  		if (webElement.getAttribute("class").contains("message-error"))
  			logger.info("异常流-编辑机构-机构显示名称重复： 操作成功！");
  		else if (webElement.getAttribute("class").contains("message-success"))
  			{
  			logger.error("异常流-编辑机构-机构显示名称重复： 操作失败！");
  			Common.TakePic();
  			}
