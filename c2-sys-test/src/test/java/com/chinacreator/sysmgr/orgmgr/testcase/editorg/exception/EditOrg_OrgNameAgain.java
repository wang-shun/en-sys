...
public class EditOrg_OrgNameAgain extends TestCase{
	Logger logger = LoggerFactory.getLogger(EditOrg_OrgNameAgain.class);
	@Test
	public void testEditOrg_OrgNameAgain() throws Exception{
		// 修改后信息
	    String orgNumber = "test";
	    String orgName = "cs";
	    String orgShowName = "test";
	    String orgXzqm = "0731";
	    String orgDesc = "编辑机构_修改机构名称和已存在重复";
	    // 选择要修改机构
...
// 		    }
		...
		if (webElement.getAttribute("class").contains("message-error"))
			logger.info("异常流-编辑机构-机构名称重复： 操作成功！");
		else if (webElement.getAttribute("class").contains("message-success"))
			{
			logger.error("异常流-编辑机构-机构名称重复： 操作失败！");
			Common.TakePic();
			}
