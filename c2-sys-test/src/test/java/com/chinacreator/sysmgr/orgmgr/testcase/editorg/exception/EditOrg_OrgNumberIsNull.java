...
public class EditOrg_OrgNumberIsNull extends TestCase{
	Logger logger = LoggerFactory.getLogger(EditOrg_OrgNumberIsNull.class);
	@Test
	public void testEditOrg_OrgNumberIsNull() throws Exception{
		// 修改后信息
	    String orgNumber = "";
	    String orgName = "test";
	    String orgShowName = "test";
	    String orgXzqm = "0731";
	    String orgDesc = "编辑机构_修改机构编码为空";
...
// 		    }
 	   ...
 	   //判断alert为正确弹框还是错误弹框
 		...
 		if (webElement.getAttribute("class").contains("message-error"))
 			logger.info("异常流-编辑机构-机构编号为空： 操作成功！");
 		else if (webElement.getAttribute("class").contains("message-success"))
 			{
 			logger.error("异常流-编辑机构-机构编号为空： 操作失败！");
 			Common.TakePic();
 			}
 	 	//关闭alert弹框
