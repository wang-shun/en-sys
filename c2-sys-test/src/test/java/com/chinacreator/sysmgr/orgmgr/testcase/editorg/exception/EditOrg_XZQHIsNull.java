...
public class EditOrg_XZQHIsNull extends TestCase{
	Logger logger = LoggerFactory.getLogger(EditOrg_XZQHIsNull.class);
	@Test
	public void testEditOrg_XZQHIsNull() throws Exception{
		// 修改后信息
	    String orgNumber = "test";
	    String orgName = "test";
	    String orgShowName = "test";
	    String orgXzqm = "";
	    String orgDesc = "编辑机构_修改行政区码为空";
...
//  		    }
 	   ...
 	   //判断alert为正确弹框还是错误弹框
   		...
   		if (webElement.getAttribute("class").contains("message-error"))
   			logger.info("异常流-编辑机构-行政区号为空： 操作成功！");
   		else if (webElement.getAttribute("class").contains("message-success"))
   			{
   			logger.error("异常流-编辑机构-行政区号为空： 操作失败！");
   			Common.TakePic();
   			}
